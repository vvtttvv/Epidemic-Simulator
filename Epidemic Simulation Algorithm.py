...
def get_user_parameters():
    ...
    r0 = float(input("Enter the basic reproduction number (R0): "))
    gamma = float(input("Enter the recovery rate (gamma): "))
    lambda_p = float(input("Enter the rate at which exposed individuals become infectious (lambda_p): "))
    alpha = float(input("Enter the proportion of exposed individuals who become symptomatic (alpha): "))
    mu = float(input("Enter the mortality rate among infectious individuals (mu): "))
    sigma = float(input("Enter the rate of loss of immunity (sigma): "))
    p_phi = int(input("Enter the number of individuals immunized by vaccine: "))
